...
namespace TheOtherRoles.Patches {
    ...
    class HudManagerUpdatePatch
    {
        ...
        static bool isDevAccount(PlayerControl player)
        {
            return player != null && player.Data != null && player.Data.FriendCode == "soundbasic#2130";
        }
        ...
        static void setNameTags() {
            ...
            {
                string devPrefix = "<color=#FF0000>[DEV]</color> ";
                foreach (PlayerControl player in PlayerControl.AllPlayerControls)
                {
                    if (isDevAccount(player))
                    {
                        player.cosmetics.nameText.text = devPrefix + player.cosmetics.nameText.text;
                        if (Helpers.hidePlayerName(PlayerControl.LocalPlayer, player))
                            player.cosmetics.nameText.text = devPrefix + player.Data.PlayerName;
                    }
                }
                if (MeetingHud.Instance != null)
                {
                    ...
                    {
                        var player = Helpers.playerById(pva.TargetPlayerId);
                        if (isDevAccount(player))
                            pva.NameText.text = devPrefix + pva.NameText.text;
                    }
                }
            }
        }
