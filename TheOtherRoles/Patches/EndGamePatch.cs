...
namespace TheOtherRoles.Patches {
    ...
    public static class OnGameEndPatch {
        ...
        public static void Postfix(AmongUsClient __instance, [HarmonyArgument(0)]ref EndGameResult endGameResult) {
            ...
            if (Leech.leech != null) notWinners.Add(Leech.leech);
            if (Collector.collector != null) notWinners.Add(Collector.collector);
            ...
            if (Leech.leech != null && Leech.target != null && !Leech.leech.Data.IsDead && EndGameResult.CachedWinners.ToArray().Any(x => x.PlayerName == Leech.target.Data.PlayerName)) {
                if (!EndGameResult.CachedWinners.ToArray().Any(x => x.PlayerName == Leech.leech.Data.PlayerName))
                    EndGameResult.CachedWinners.Add(new CachedPlayerData(Leech.leech.Data));
            }

            if (Collector.collector != null && Collector.targetWasCollected()) {
                if (!EndGameResult.CachedWinners.ToArray().Any(x => x.PlayerName == Collector.collector.Data.PlayerName))
                    EndGameResult.CachedWinners.Add(new CachedPlayerData(Collector.collector.Data));
            }
