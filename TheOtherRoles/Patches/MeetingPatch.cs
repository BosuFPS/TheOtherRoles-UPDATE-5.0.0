    ...
    class MeetingHudPatch {
        ...
        class StartMeetingPatch {
            public static void Prefix(PlayerControl __instance, [HarmonyArgument(0)]NetworkedPlayerInfo meetingTarget) {
                ...
                if (Leech.leech != null && PlayerControl.LocalPlayer == Leech.leech && Leech.target != null && !Leech.leech.Data.IsDead) {
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(Leech.leech, $"Your host is {Leech.target.Data.PlayerName}.");
                }

                if (Collector.collector != null && PlayerControl.LocalPlayer == Collector.collector && Collector.target != null && !Collector.collector.Data.IsDead) {
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(Collector.collector, $"Your target is {Collector.target.Data.PlayerName}. You collect if they die by someone else.");
                }

                if (Archivist.archivist != null && PlayerControl.LocalPlayer == Archivist.archivist && !Archivist.archivist.Data.IsDead) {
                    List<DeadPlayer> newDeaths = GameHistory.deadPlayers.Skip(Archivist.lastKnownDeadCount).ToList();
                    int kills = newDeaths.Count(x => x.deathReason == DeadPlayer.CustomDeathReason.Kill || x.deathReason == DeadPlayer.CustomDeathReason.Bomb || x.deathReason == DeadPlayer.CustomDeathReason.Arson || x.deathReason == DeadPlayer.CustomDeathReason.WitchExile);
                    string names = newDeaths.Count == 0 ? "Nobody" : string.Join(", ", newDeaths.Select(x => x.player.Data.PlayerName));
                    bool killerAlive = PlayerControl.AllPlayerControls.ToArray().Any(p => !p.Data.IsDead && !p.Data.Disconnected && Helpers.isKiller(p));
                    string msg = $"Archive since last meeting:\nDeaths: {names}\nKills: {kills}\nKiller alive: {(killerAlive ? "Yes" : "No")}";
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(Archivist.archivist, msg);
                    Archivist.lastKnownDeadCount = GameHistory.deadPlayers.Count;
                }
