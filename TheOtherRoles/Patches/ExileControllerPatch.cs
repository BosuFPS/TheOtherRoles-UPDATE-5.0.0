...
namespace TheOtherRoles.Patches {
    ...
    class ExileControllerWrapUpPatch {
        ...
        static void WrapUpPostfix(PlayerControl exiled) {
            ...
            if (Sleepwalker.sleepwalker != null && PlayerControl.LocalPlayer == Sleepwalker.sleepwalker && !Sleepwalker.sleepwalker.Data.IsDead) {
                List<PlainShipRoom> rooms = new List<PlainShipRoom>();
                foreach (PlainShipRoom room in MapUtilities.CachedShipStatus.FastRooms.Values) {
                    if (room != null && room.roomArea != null) rooms.Add(room);
                }
                if (rooms.Count > 0) {
                    PlainShipRoom room = rooms[TheOtherRoles.rnd.Next(0, rooms.Count)];
                    Vector3 position = room.roomArea.bounds.center;
                    position.z = Sleepwalker.sleepwalker.transform.position.z;
                    Sleepwalker.sleepwalker.NetTransform.RpcSnapTo(position);
                }
            }
