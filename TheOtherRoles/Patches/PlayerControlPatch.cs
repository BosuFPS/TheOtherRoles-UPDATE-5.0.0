...
namespace TheOtherRoles.Patches {
    ...
    public static class PlayerControlFixedUpdatePatch {
        ...
        static void snitchUpdate() {
            ...
            float taskProgress = (float)playerCompleted / playerTotal;

            if (!snitchIsDead && local == Snitch.snitch) {
                if (!Snitch.hasKillerInfo && taskProgress >= 0.5f) {
                    List<PlayerControl> possibleTargets = PlayerControl.AllPlayerControls.ToArray().ToList()
                        .Where(p => !p.Data.IsDead && p != Sheriff.sheriff && Helpers.isKiller(p))
                        .ToList();

                    string msg = "Snitch info: No killing role found.";
                    if (possibleTargets.Count > 0) {
                        PlayerControl target = possibleTargets[TheOtherRoles.rnd.Next(0, possibleTargets.Count)];
                        msg = $"Snitch info: {target.Data.PlayerName} is a killing role.";
                    }
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(Snitch.snitch, msg);
                    Snitch.hasKillerInfo = true;
                }

                if (!Snitch.hasArmor && taskProgress >= 0.75f) {
                    MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(Snitch.snitch.NetId, (byte)CustomRPC.SetSnitchArmor, Hazel.SendOption.Reliable, -1);
                    writer.Write(Snitch.snitch.PlayerId);
                    AmongUsClient.Instance.FinishRpcImmediately(writer);
                    RPCProcedure.setSnitchArmor(Snitch.snitch.PlayerId);
                }
            }
