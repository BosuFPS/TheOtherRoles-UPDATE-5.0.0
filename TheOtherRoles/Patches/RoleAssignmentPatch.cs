...
namespace TheOtherRoles.Patches
{
    ...
    class RoleManagerSelectRolesPatch {
        ...
        public static RoleAssignmentData getRoleAssignmentData() {
            ...
            neutralSettings.Add((byte)RoleId.Leech, CustomOptionHolder.leechSpawnRate.getSelection());
            neutralSettings.Add((byte)RoleId.Collector, CustomOptionHolder.collectorSpawnRate.getSelection());
            ...
            crewSettings.Add((byte)RoleId.Sleepwalker, CustomOptionHolder.sleepwalkerSpawnRate.getSelection());
            crewSettings.Add((byte)RoleId.Locksmith, CustomOptionHolder.locksmithSpawnRate.getSelection());
            crewSettings.Add((byte)RoleId.Archivist, CustomOptionHolder.archivistSpawnRate.getSelection());
        ...
        public static void assignRoleTargets(RoleAssignmentData data) {
            ...
            if (Leech.leech != null) {
                var possibleTargets = PlayerControl.AllPlayerControls.ToArray().ToList();
                possibleTargets.RemoveAll(p => p == null || p.Data.IsDead || p.Data.Disconnected || p.PlayerId == Leech.leech.PlayerId);
                if (possibleTargets.Count > 0) {
                    PlayerControl target = possibleTargets[TheOtherRoles.rnd.Next(0, possibleTargets.Count)];
                    MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SetLeechTarget, Hazel.SendOption.Reliable, -1);
                    writer.Write(target.PlayerId);
                    AmongUsClient.Instance.FinishRpcImmediately(writer);
                    RPCProcedure.setLeechTarget(target.PlayerId);
                }
            }

            if (Collector.collector != null) {
                var possibleTargets = PlayerControl.AllPlayerControls.ToArray().ToList();
                possibleTargets.RemoveAll(p => p == null || p.Data.IsDead || p.Data.Disconnected || p.PlayerId == Collector.collector.PlayerId || (!p.Data.Role.IsImpostor && p != Jackal.jackal && p != Sidekick.sidekick));
                if (possibleTargets.Count > 0) {
                    PlayerControl target = possibleTargets[TheOtherRoles.rnd.Next(0, possibleTargets.Count)];
                    MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SetCollectorTarget, Hazel.SendOption.Reliable, -1);
                    writer.Write(target.PlayerId);
                    AmongUsClient.Instance.FinishRpcImmediately(writer);
                    RPCProcedure.setCollectorTarget(target.PlayerId);
                }
            }
        }
