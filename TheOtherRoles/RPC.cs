...
namespace TheOtherRoles
{
    ...
    public static void HandleShareOptions(byte numberOfOptions, MessageReader reader) {            
            ...
            foreach (PlayerControl player in PlayerControl.AllPlayerControls) {
                if (player.PlayerId == playerId) {
                    ...
                    case RoleId.Sleepwalker:
                        Sleepwalker.sleepwalker = player;
                        break;
                    case RoleId.Leech:
                        Leech.leech = player;
                        break;
                    case RoleId.Collector:
                        Collector.collector = player;
                        break;
                    case RoleId.Locksmith:
                        Locksmith.locksmith = player;
                        break;
                    case RoleId.Archivist:
                        Archivist.archivist = player;
                        break;
                    }
        ...
        public static void setSnitchArmor(byte playerId) {
            if (Snitch.snitch == null || Snitch.snitch.PlayerId != playerId) return;
            Snitch.hasArmor = true;
            Snitch.isBrokenArmor = false;
        }

        public static void breakSnitchArmor() {
            if (Snitch.snitch == null || !Snitch.hasArmor || Snitch.isBrokenArmor) return;
            Snitch.isBrokenArmor = true;
            if (PlayerControl.LocalPlayer.Data.IsDead) {
                Snitch.snitch.ShowFailedMurder();
            }
        }

        public static void setLeechTarget(byte targetId) {
            Leech.target = Helpers.playerById(targetId);
        }

        public static void setCollectorTarget(byte targetId) {
            Collector.target = Helpers.playerById(targetId);
        }

        public static void locksmithUsed() {
            Locksmith.remainingUses = Mathf.Max(0, Locksmith.remainingUses - 1);
        }
    }
        ...
        static void Postfix([HarmonyArgument(0)]byte callId, [HarmonyArgument(1)]MessageReader reader)
        {
            ...
            switch (packetId) {
                ...
                case (byte)CustomRPC.BreakArmor:
                    ...
                    break;
                case (byte)CustomRPC.SetSnitchArmor:
                    RPCProcedure.setSnitchArmor(reader.ReadByte());
                    break;
                case (byte)CustomRPC.BreakSnitchArmor:
                    RPCProcedure.breakSnitchArmor();
                    break;
                case (byte)CustomRPC.SetLeechTarget:
                    RPCProcedure.setLeechTarget(reader.ReadByte());
                    break;
                case (byte)CustomRPC.SetCollectorTarget:
                    RPCProcedure.setCollectorTarget(reader.ReadByte());
                    break;
                case (byte)CustomRPC.LocksmithUsed:
                    RPCProcedure.locksmithUsed();
                    break;
