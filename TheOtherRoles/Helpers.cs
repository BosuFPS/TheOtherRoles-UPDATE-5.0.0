...
namespace TheOtherRoles {
    ...
    public static class Helpers
    {
        ...
        public static bool checkArmored(PlayerControl target, bool breakShield, bool showShield, bool additionalCondition = true) {
            ...
            if (target != null && Snitch.snitch != null && Snitch.snitch == target && Snitch.hasArmor && !Snitch.isBrokenArmor && additionalCondition) {
                if (breakShield) {
                    MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.BreakSnitchArmor, Hazel.SendOption.Reliable, -1);
                    AmongUsClient.Instance.FinishRpcImmediately(writer);
