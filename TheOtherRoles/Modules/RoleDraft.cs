...
namespace TheOtherRoles.Modules
{
    ...
    class RoleDraft
    {
        ...
        private static Sprite getDraftButtonSprite(RoleInfo roleInfo, Sprite fallback) {
            switch (roleInfo.roleId) {
                case RoleId.Leech:
                    return Vulture.getButtonSprite();
                case RoleId.Collector:
                    return Cleaner.getButtonSprite();
                case RoleId.Locksmith:
                    return Locksmith.getButtonSprite();
                case RoleId.Archivist:
                    return Medium.getQuestionSprite();
                default:
                    return fallback;
            }
        }
                    ...
                    if (PlayerControl.LocalPlayer.PlayerId == pickOrder[0]) {
                        ...
                        if (GameObject.Find("RoleButton") == null) {
                            ...
                            foreach (RoleInfo roleInfo in availableRoles) {
                                ...
                                if (roleInfo.roleId == RoleId.Sleepwalker)
                                    actionButtonRenderer.enabled = false;
                                else
                                    actionButtonRenderer.sprite = getDraftButtonSprite(roleInfo, actionButtonRenderer.sprite);
