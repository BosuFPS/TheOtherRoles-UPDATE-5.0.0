...
namespace TheOtherRoles
{
    ...
    static class HudManagerStartPatch
    {
        ...
        private static CustomButton locksmithButton;
        ...
        public static void createButtonsPostfix(HudManager __instance) {
            ...
            locksmithButton = new CustomButton(
                () => {
                    MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.LocksmithUsed, Hazel.SendOption.Reliable, -1);
                    AmongUsClient.Instance.FinishRpcImmediately(writer);
                    RPCProcedure.locksmithUsed();

                    for (byte doorId = 0; doorId < 64; doorId++) {
                        ShipStatus.Instance.RpcRepairSystem(SystemTypes.Doors, doorId);
                    }
                    SoundEffectsManager.play("engineerRepair");
                },
                () => { return Locksmith.locksmith != null && Locksmith.locksmith == PlayerControl.LocalPlayer && Locksmith.remainingUses > 0 && !PlayerControl.LocalPlayer.Data.IsDead; },
                () => { return Locksmith.remainingUses > 0 && PlayerControl.LocalPlayer.CanMove; },
                () => { },
                Locksmith.getButtonSprite(),
            ...
            // Add "edited by BosuFPS" text during gameplay
            var editorText = new GameObject("EditedByText").AddComponent<TMPro.TextMeshPro>();
            editorText.transform.SetParent(__instance.transform, false);
            editorText.fontSize = 1f;
            editorText.fontStyle = TMPro.FontStyles.Bold;
            editorText.alignment = TMPro.TextAlignmentOptions.BottomRight;
            editorText.text = "<color=#FF0000>edited by BosuFPS</color>";
            editorText.color = new Color(1f, 0.2f, 0.2f, 0.5f);
            editorText.transform.localPosition = new Vector3(3.5f, 0.5f, -10f);
